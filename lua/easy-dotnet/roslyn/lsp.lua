...
-- Add this BEFORE your on_attach setup
local function refresh_codelens_silent(bufnr)
    local params = { textDocument = vim.lsp.util.make_text_document_params(bufnr) }
    local client = vim.lsp.get_clients({ bufnr = bufnr, method = "textDocument/codeLens" })[1]
    ...
    client:request("textDocument/codeLens", params, function(err, result)
        ...
        if remaining == 0 then
            vim.lsp.codelens.display(result, bufnr, client.id)
            return
        end

        -- Step 2: Manually resolve each lens before displaying
        for _, lens in ipairs(result) do
            client:request("codeLens/resolve", lens, function(_, resolved_lens)
                -- Update the lens in our list with the resolved data
                if resolved_lens then
                    lens.command = resolved_lens.command
                end

                remaining = remaining - 1
                -- Only update the UI once the LAST lens is resolved
                if remaining == 0 then
                    vim.lsp.codelens.display(result, bufnr, client.id)
                end
            end)
...
function M.find_project_or_solution(bufnr, cb)
    coroutine.wrap(function()
        ...
        require("easy-dotnet.picker").picker(nil, vim.tbl_map(function(value) return { display = value } end, sln),
            function(r)
...
function M.find_sln_or_csproj(dir)
    local slns = vim.tbl_map(function(val) return vim.fs.normalize(val) end,
        vim.fs.find(function(name) return name:match("%.slnx?$") end, { path = dir, upward = false, limit = 100 }))
    ...
    vim.api.nvim_create_autocmd("BufReadCmd", {
        ...
        callback = function(args)
            ...
            for _, client in ipairs(clients) do
                if does_file_belong_to_active_client(client, args.buf) then
                    populate_source_generated_buffer(client,
                        args.buf, args.file)
                end
            end
        end,
...
function M.enable(opts)
    ...
    local settings = vim.tbl_deep_extend("force", default_roslyn_settings, opts.config.settings or {},
        existing_config and existing_config.settings or {})
    ...
    vim.lsp.config[constants.lsp_client_name] = {
        ...
        on_init = function(client)
            ...
            if type == "sln" then
                M.state[client.id] = job.register_job({
                    name = "Opening solution",
                    on_error_text =
                    "Failed to open solution",
                    on_success_text = "Workspace ready",
                    timeout = 150000
                })
                            ...
                            registration.registerOptions.watchers = vim
                                ...
                                :filter(function(watch)
                                    if type(watch.globPattern) == "table" and watch.globPattern.baseUri then
                                        return vim
                                            .loop.fs_stat(vim.uri_to_fname(watch.globPattern.baseUri)) ~= nil
                                    end
            ...
            ["workspace/refreshSourceGeneratedDocument"] = function(_, _, ctx)
                ...
                for _, buf in ipairs(vim.api.nvim_list_bufs()) do
                    ...
                    if vim.api.nvim_buf_get_name(buf):match("^roslyn%-source%-generated://") then
                        populate_source_generated_buffer(client, buf, uri)
                    end
                end
            end,
        },
